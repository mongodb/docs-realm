...
1. **Close all instances of the realm**: Close *all open instances* of
...
1. **Move the realm to a backup file**: Call `executeClientReset()`
   on the provided [`ClientResetRequiredError`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/ClientResetRequiredError.html#getBackupFile()).
...
1. **Download all realm data from the backend**: Download the entire set
   ...
   doesn't specify the [`waitForInitialRemoteData()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/SyncConfiguration.Builder.html#waitForInitialRemoteData(long,java.util.concurrent.TimeUnit))
   option, you can call [`syncSession.downloadAllServerChanges()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/SyncSession.html#downloadAllServerChanges(long,java.util.concurrent.TimeUnit))
   after opening the realm.

1. **Open the realm backup**: Use the [`getBackupRealmConfiguration()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/ClientResetRequiredError.html#getBackupRealmConfiguration())
   ...
   this instance as a [`DynamicRealm`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/DynamicRealm.html),
...
strategy,  pass an instance of [`ManuallyRecoverUnsyncedChangesStrategy`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/ManuallyRecoverUnsyncedChangesStrategy.html)
to the [`defaultSyncClientResetStrategy()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/AppConfiguration.Builder.html#defaultSyncClientResetStrategy(io.realm.mongodb.sync.DiscardUnsyncedChangesStrategy))
...
<Tabs groupId="java-sdk-languages">
    <TabItem value="kotlin" label="Kotlin">
        <CodeBlock title="Potato.kt" language="kotlin">{PotatoKotlin}</CodeBlock>
        <CodeBlock title="Onion.kt" language="kotlin">{OnionKotlin}</CodeBlock>
        <CodeBlock title="Rice.kt" language="kotlin">{RiceKotlin}</CodeBlock>
    </TabItem>
    <TabItem value="java" label="Java">
        <CodeBlock title="Potato.java" language="java">{PotatoJava}</CodeBlock>
        <CodeBlock title="Onion.java" language="java">{OnionJava}</CodeBlock>
        <CodeBlock title="Rice.java" language="java">{RiceJava}</CodeBlock>
    </TabItem>
</Tabs>
...
<Tabs groupId="java-sdk-languages">
    <TabItem value="kotlin" label="Kotlin">
        <CodeBlock title="LastSynced.kt" language="kotlin">{LastSyncedKotlin}</CodeBlock>
    </TabItem>
    <TabItem value="java" label="Java">
        <CodeBlock title="LastSynced.java" language="java">{LastSyncedJava}</CodeBlock>
    </TabItem>
</Tabs>

You can use [`syncSession.addUploadProgressListener()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/SyncSession.html#addUploadProgressListener(io.realm.mongodb.sync.ProgressMode,io.realm.mongodb.sync.ProgressListener))
to listen for upload progress events in your `App`. Implement
`onChange()` to handle these events. Call
[`Progress.isTransferComplete()`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/Progress.html#isTransferComplete())
...
Register your progress listener with [`ProgressMode.INDEFINITELY`](https://docs.mongodb.com/realm-sdks/java/latest/io/realm/mongodb/sync/ProgressMode.html#INDEFINITELY) to subscribe
