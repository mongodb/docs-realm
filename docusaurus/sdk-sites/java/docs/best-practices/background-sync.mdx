...
Background sync requires two things: synchronization logic, and the job
that periodically performs that logic.

### Synchronization Logic

First, write the custom logic that synchronizes your realm. Treat this
logic as a standalone connection to your backend, including:

- initializing the Realm SDK
- an authenticated user to open the realm
...
Put your synchronization logic in the `startWork()` method:
