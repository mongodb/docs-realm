...
// Note: the following is not really tested but it makes for a convenient
// full program example.
// :code-block-start: register-a-partial-keypath-change-listener
// Define the dog class.
class NotificationExamplePartialKeyPath_Dog: Object {
    @Persisted var name = ""
    @Persisted var favoriteToy = ""
    @Persisted var age: Int?
}

var partialKeyPathObjectNotificationToken: NotificationToken?

func partialKeyPath_objectNotificationExample() {
    let dog = NotificationExampleKeyPath_Dog()
    dog.name = "Max"
    dog.favoriteToy = "Ball"
    dog.age = 2
    ...
    // reference to the notification token or the observation will stop.
    // Invalidate the token when done observing.
    objectNotificationToken = dog.observe(keyPaths: [\NotificationExamplePartialKeyPath_Dog.favoriteToy, \NotificationExamplePartialKeyPath_Dog.age], { change in // :emphasize:
        switch change {
        case .change(let object, let properties):
            for property in properties {
                print("Property '\(property.name)' of object \(object) changed to '\(property.newValue!)'")
