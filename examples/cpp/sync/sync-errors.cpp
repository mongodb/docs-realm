...
TEST_CASE("beta compensating write error outside query", "[error]") {
  ...
  auto simpleItem =
      Beta_SyncError_Item{.ownerId = user.identifier(),
  ...
  auto complexItem = Beta_SyncError_Item{._id = primaryKey,
  ...
  for (auto &v : info) {
    ...
    REQUIRE(v.reason == "write to ObjectID(\"" + pkString +
                            "\") in table \"Beta_SyncError_Item\" not allowed; "
...
TEST_CASE("beta compensating write error write doesn't match permissions",
          "[error]") {
  auto appConfig = realm::App::configuration();
  appConfig.app_id = APP_ID;
  auto app = realm::App(appConfig);
