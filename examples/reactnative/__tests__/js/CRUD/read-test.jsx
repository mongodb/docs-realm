...
describe("Read Data Tests", () => {
    ...
    it("should read from a realm", async () => {
        ...
        const App = () => <RealmProvider> <TaskItem _id={142339}/> </RealmProvider>
        ...
        const taskItemSentence = await waitFor(() => getByTestId("task-item-sentence")); 
    ...
    it("should filter data", async () => {
            ...
            <>
                <Text>Your high priority tasks:</Text>
                {
                    highPriorityTasks.map((taskItem, i) => {
                        return(<Text key={taskItem._id + 'element' + i} testID='high-priority-element'>{taskItem.name}</Text>)
                    })
                }
                <Text>Your tasks without much progress:</Text>
                {
                    lowProgressTasks.map((taskItem, i) => {
                        return(<Text key={taskItem._id + 'element' + i} testID='low-progress-element'>{taskItem.name}</Text>)
                    })
                }
            </>)
        }
        ...
        const App = () => <RealmProvider> <TaskList /> </RealmProvider>
        const { getByTestId, getAllByTestId, getByText} = render(<App />);

        // test that the highPriorityTasks items Text renders
        const highPriorityTasksUIList = await waitFor(() => getAllByTestId("high-priority-element")); 
        ...
        // test that the highPriorityTasks items Text renders
        const lowProgressTasksUIList = await waitFor(() => getAllByTestId("low-progress-element")); 
    ...
    it("should render sorted tasks", async () => {
        ...
        const TaskList = () => {
            ...
            <>
            <Text>All tasks:</Text>
            { tasks.map((task) => <Text testID='all-tasks-item' key={"all-tasks-" + task._id}>{task.name}</Text>)}

            <Text>Tasks sorted by name:</Text>
            { tasksByName.map((task) => <Text testID='tasks-by-name-item' key={"tasks-by-name-" + task._id}>{task.name}</Text>)}

            <Text>Tasks sorted by name descending:</Text>
            { tasksByNameDescending.map((task) => <Text testID='tasks-by-name-descending-item' key={"tasks-by-name-desc-" + task._id}>{task.name}</Text>)}

            <Text>Tasks sorted by priority descending, and name alphabetically:</Text>
            { tasksByPriorityDescendingAndName.map((task) => <Text testID='tasks-by-priority-descending-and-name-item' key={"tasks-by-priority-descending-and-name-" + task._id}>{task.name}</Text>)}

            <Text>Tasks sorted by assignee name:</Text>
            { tasksByAssigneeName.map((task) => <Text testID='tasks-by-assignee-name-item' key={"tasks-by-assignee-name-" + task._id}>{task.name}</Text>)}
        ...
        const allTasksUIList = await waitFor(() => getAllByTestId("all-tasks-item"));
        ...
        const taskByNameUIList = await waitFor(() => getAllByTestId("tasks-by-name-item"));
        ...
        const taskByNameDescendingUIList = await waitFor(() => getAllByTestId("tasks-by-name-descending-item"));
        ...
        // test that tasksByNameDescending should be in reverse alphabetical name order
        const tasksByAssigneeNameUIList = await waitFor(() => getAllByTestId("tasks-by-assignee-name-item"));
