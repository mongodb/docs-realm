import Realm, { BSON, Credentials, CompensatingWriteError } from "realm";
  ...
  test("compensating write error", async () => {
    ...
    const errorCallback = (session, error) => {
      ...
      if (error instanceof CompensatingWriteError) {
        ...
        expect(compensatingWrites[0].primaryKey.equals(personId)).toBe(true);
        expect(compensatingWrites[1].primaryKey.equals(turtleId)).toBe(true);
