const TaskList = () => {
    ...
    <>
        ...
        { tasksByName.map((task) => <Text>{task.name}</Text>)}

        <Text>Tasks sorted by name descending:</Text>
        { tasksByNameDescending.map((task) => <Text>{task.name}</Text>)}

        <Text>Tasks sorted by priority descending, and name alphabetically:</Text>
        { tasksByPriorityDescendingAndName.map((task) => <Text>{task.name}</Text>)}

        <Text>Tasks sorted by assignee name:</Text>
        { tasksByAssigneeName.map((task) => <Text>{task.name}</Text>)}
    </>
    )
}
